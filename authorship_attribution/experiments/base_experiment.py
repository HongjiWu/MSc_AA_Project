...
class BaseExperiment:
    ...
    def compute_accuracies(self, predictions, test_labels):
        ...
        top_1 = np.mean([(predictions[i, 0:1] == test_labels[i]).any() for i in range(len(test_labels))])
        top_5 = np.mean([(predictions[i, 0:5] == test_labels[i]).any() for i in range(len(test_labels))])
        top_10 = np.mean([(predictions[i, 0:10] == test_labels[i]).any() for i in range(len(test_labels))])
        top_100 = np.mean([(predictions[i, 0:100] == test_labels[i]).any() for i in range(len(test_labels))])
        ...
        for iter_num in tqdm(range(repeats)):
            try:
                ...
                if self.author_pool is not None:
                    training_data, testing_data, self.author_pool =  self.reader.subsample_split_df(num_authors=num_authors,
                ...
                if self.params.output_file:
                    ...
                    if self.params.store_result:
                        ...
                        with open(self.params.output_file, "a") as file:
                            file.writelines(
                                [f"{id_}, {pred}, {succ}, {run_id}\n" for id_, pred, succ in
                                 zip(test_ids, predictions, (predictions == test_labels).astype(int))])
                        ...
                        with open(self.params.output_file, "a") as file:
                            file.writelines(
                                [f"{id_}, {pred}, {run_id}\n" for id_, pred in
                                 zip(test_ids, (predictions == test_labels).astype(int))])
