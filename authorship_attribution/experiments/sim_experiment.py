...
from authorship_attribution.data_loader.sim_reader import DataReader
...
class SimExperiment:
    # This experiment is designed to test performance of AA_method when changing similarity between training and testing samples.
    ...
    def compute_accuracies(self, predictions, test_labels):
        ...
        if isinstance(self.aa_method, BertHybridMethod):
    ...
    def fit_model(self, model, train_ids, test_features, test_labels, test_ids, acc, acc_train, exp_type, num_authors):
        ...
        if self.params.use_gap_statistic:
            ...
            if not isinstance(self.aa_method, BertHybridMethod):
    ...
    def experiment_run(self, repeats=1, num_authors=50, sim_metric = "feature", sim_level = "middle"):
        """
        ...
        for iter_num in tqdm(range(repeats)):
            try:
                ...
                self.fit_model( model, train_ids, aux_test_features,  aux_test_labels, aux_test_ids, acc_cross, acc_train, sim_metric + '_' + sim_level, num_authors)
                return


                for sim_metric in ['feature', 'bert']:
                    ...
                    for sim_level in ['low', 'middle', 'high']:
                        ...
                        training_data, testing_data, aux_data, author =  self.reader.subsample_split_df(num_authors=num_authors,
                            domain=self.params.domains,
                            random_seed=iter_num if self.params.set_deterministic else None,
    ...
    # def plot(self, repeats, acc, num_authors):
    #     # Display experimental stats
    #     logging.info(f"Mean accuracy ({repeats} repeats): {np.array(acc).mean()}")
    #     plt.hist(acc, bins=10)
    #     # plt.boxplot(acc, bootstrap=1000)
    #     plt.title(
    #         f"{self.__class__.__name__}:{self.aa_method.name} - {num_authors} authors - {self.reader.min_samples_per_author} samples/author",
    #         fontsize=10)
    #     plt.xlabel(
    #         f"Accuracy over test set ({repeats} runs) - mean: {np.array(acc).mean():.3f} - Random baseline: {1 / num_authors}")
