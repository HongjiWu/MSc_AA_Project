...
class NaiveTripletLoss(nn.Module):
    def __init__(self, margin=1.0):
        """
        Naive triplet loss implementation (random samples)
        :param margin: margin parameter for triplet loss
        """
        super().__init__()
        self.margin = margin
...
class OnlineTripletLoss(nn.Module):
    def __init__(self, margin=1.0, batch_hard: bool = True, device="cuda:0"):
        """

        :param margin: margin parameter for triplet loss
        :param batch_hard: True: Apply hard batching strategy (closest negative, farthest positive).
                           False: Apply batch all strategy.
        :param device: Device to push tensors to (cuda or cpu)
        """
        super().__init__()
        self.margin = margin
        self.device = device
    ...
    def forward(self, *args, **kwargs):
        if self.batch_hard:
            return self.batch_hard_triplet_loss(*args, **kwargs)
        return self.batch_all_triplet_loss(*args, **kwargs)
